...
Select Case Action
Case "Up"
	MinOrder=Conn.Execute("Select Min(NavOrder) From FirendLink Where NavParent="&NavParent&"")(0)
	...
	Else
		Conn.Execute("Update FirendLink Set NavOrder="&NavOrder&" Where NavOrder="&NavOrder-1&" And Navparent="&Navparent&"")
		Conn.Execute("Update FirendLink Set NavOrder="&NavOrder-1&" Where ID="&ID&"")
	...
	Response.End()
Case "Down"
	MaxOrder=Conn.Execute("Select Max(NavOrder) From FirendLink Where NavParent="&NavParent&"")(0)
	NavOrder=ReplaceBadChar(Trim(Request("NavOrder")))
	...
	Response.End()
Case "UnPublic"
	Conn.Execute("Update FirendLink Set NavLock=1 Where ID="&ID&"")
	Response.Redirect(Request.ServerVariables("HTTP_REFERER"))
	Response.End()
Case "InPublic"
	Conn.Execute("Update FirendLink Set NavLock=0 Where ID="&ID&"")
	Response.Redirect(Request.ServerVariables("HTTP_REFERER"))
	Response.End()
Case "Delete"
	...
	For i = LBound(AryID) To UBound(AryID)
		If IsNumeric(AryID(i))=True Then
			Conn.Execute("Delete From FirendLink Where ID In ("&AryID(i)&GetAllChild("FirendLink",AryID(i))&")")
		End If
	Next
	Conn.Close
	Set Conn=Nothing
...
<tr>
<td width="92%" style="border-bottom:solid 1px #dde4e9;height:30px;width:80%">当前位置：<a href="FirendLink.asp">有情链接维护</a></td>
<td width="8%" style="border-bottom:solid 1px #dde4e9;height:30px;width:20%; text-align:right"><a href="FirendLinkAdd.asp?ID=<%=ID%>">添加内容</a>&nbsp;</td>
</tr>
<tr>
...
<td width="90%" valign="top">1.以下为系统所有单页内容的信息列表；<br />
  2.单击[+]可添加友情链接；<br>
注意：您可以进行添加、修改、删除等操作，保存之后立即生效。 类别：1 代表：商会网站链接； 2 代表：会员网站链接</td>
</tr>
</table></td>
</tr>
<tr>
...
<th width="26%" class="Right">链接标题或链接图片<a href="FirendLinkAdd.asp?ID=<%=ID%>">[+]</a></th>
<th width="28%" class="Right">链接地址</th>
<th width="8%" class="Right">类别</th>
<th width="8%" class="Right">排序</th>
<th width="10%" class="Right">状态</th>
<th width="8%">管理操作</th>
</tr>
<%
Set Rs=Server.CreateObject("Adodb.RecordSet")
Sql="Select * From FirendLink   Order By NavOrder desc"
...
<td class="Right"><a href="FirendLinkEdit.asp?ID=<%=Rs("ID")%>&Page=<%=Page%>&ParentID=<%=ID%>">
  <%if instr(Trim(Rs("LinkTitleOrPic")),"UploadFile")>0 then%>
  <img src="<%=Rs("LinkTitleOrPic")%>" width="88" height="31" border="0" />
  <%else%>
  <%Response.Write(Rs("NavContent"))%>
  <%end if%>
</a></td>
<td class="Right"><a href="<%=Rs("LinkAddress")%>" target="_blank"><%=Rs("LinkAddress")%></a>&nbsp;</td>
<td class="Right"> <%=Rs("navparent")%> &nbsp;</td>
<td class="Right"><a href="?Action=Down&ID=<%=Rs("ID")%>&NavOrder=<%=Rs("NavOrder")%>&NavParent=<%=Rs("NavParent")%>">上移</a>┊<a href="?Action=Up&ID=<%=Rs("ID")%>&NavOrder=<%=Rs("NavOrder")%>&NavParent=<%=Rs("NavParent")%>">下移</a></td>
<td class="Right">
  <%
...
End If
%></td>
<td><a href="FirendLinkEdit.asp?ID=<%=Rs("ID")%>&Page=<%=Page%>&ParentID=<%=ID%>">编辑</a>┊<a href="?Action=Delete&ID=<%=Rs("ID")%>&Page=<%=Page%>&ParentID=<%=ID%>" onclick="if(!confirm('\u786e\u8ba4\u8981\u5c06\u8be5\u5bfc\u822a\u6761\u4fe1\u606f\u5220\u9664\u5417\uff1f')) return false;">删除</a></td>
</tr>
<%
i=i+1
Rs.MoveNext
