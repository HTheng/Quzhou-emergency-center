...
if request("action")="Delete" then
 ...
 Conn.Execute("Delete * From order_file Where ID="&ID)
 end if
%>
...
<th colspan="2" class="Right">订单编号<a href="Order_Addinfo.asp?ID=<%=ID%>">[+]</a></th>
<th width="32%" class="Right">时间</th>
<th width="13%">管理操作</th>
</tr>
<%
Set Rs=Server.CreateObject("Adodb.RecordSet")
Sql="Select * From order_file Order By posttime Asc"
...
<td colspan="2" class="Right"><a href="Order_infoContentEdit.asp?ID=<%=Rs("ID")%>&Page=<%=Page%>">
<%	 
...
<td><a href="Order_infoContentEdit.asp?ID=<%=Rs("ID")%>&Page=<%=Page%>">编辑</a>┊<a href="?Action=Delete&ID=<%=Rs("ID")%>&Page=<%=Page%>&ParentID=<%=ID%>" onclick="if(!confirm('\u786e\u8ba4\u8981\u5c06\u8be5\u4fe1\u606f\u5206\u7c7b\u5220\u9664\u5417\uff1f')) return false;">删除</a></td>
</tr>
<%
i=i+1
Rs.MoveNext
