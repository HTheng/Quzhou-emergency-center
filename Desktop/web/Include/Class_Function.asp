...
<%
Function LoseHtml(nr)
 Dim ClsTempLoseStr,regEx
 ClsTempLoseStr = Cstr(nr)
...
Function ISPopedom(UserName,Popedom)
	If ISAdmin(UserName)=False Then
		...
		If Rs.Eof Or Rs.Bof Then
			...
			Response.Write("</tr>")&chr(13)
			Response.Write("<tr>")&chr(13)
			Response.Write("<td style=""color:#808080"">如果您有任何疑问欢迎致电捷点科技客服部进行咨询。<br/>客服热线：0570-8068882<br/>客服邮箱：Service@Cnvp.Com.Cn")&chr(13)
...
Function GetPageNavPath(Table,ID)
	...
	If Not (Rs.Eof Or Rs.Bof) Then
		Str= " > <a href=""?ID="&Rs("ID")&""">" & Rs("User_NavTitle") &"</a>"
...
Function GetPageNavPath4(Table,ID)
	Set Rs=Conn.Execute("Select ID,User_EnNavTtile,User_NavParent From "&Table&" Where ID="&ID&"")
	If Not (Rs.Eof Or Rs.Bof) Then
		Str= " > " & Rs("User_EnNavTtile") 
		Str=GetPageNavPath4(Table,Rs("User_NavParent")) & Str
	End if
	GetPageNavPath4=Str
End Function
'********************************
...
Function GetPageNavName(Table,classid)
	GetPageNavName=Conn.Execute("Select User_NavTitle From "&Table&" Where ID="&classid&"")(0)
End Function
...
Function GetPostTimey(ID,TableName)
Dim Rs,Str
Set Rs=Conn.Execute("Select PostTime From "&TableName&" Where ID="&ID)
Str=""&Right("0"&Month(Rs("PostTime")),2)&"-"&Right("0"&Day(Rs("PostTime")),2)&""
Rs.Close
Set Rs=Nothing
GetPostTimey=Str
End Function

Function GetPostTimenews(ID,TableName)
Dim Rs,Str
Set Rs=Conn.Execute("Select PostTime From "&TableName&" Where ID="&ID)
Str=""&Right(Year(Rs("PostTime")),4)&"/"&Right("0"&Month(Rs("PostTime")),2)&"/"&Right("0"&Day(Rs("PostTime")),2)&""
Rs.Close
...
Function GetPostTime1d(ID,TableName)
Dim Rs,Str
Set Rs=Conn.Execute("Select PostTime From "&TableName&" Where ID="&ID)
Str="("&Right(Year(Rs("PostTime")),4)&"-"&Right("0"&Month(Rs("PostTime")),2)&"-"&Right("0"&Day(Rs("PostTime")),2)&")"
Rs.Close
...
'返回值：2008-08-08
'**************************************************
Function GetPostTimeL(ID)
Dim Rs,Str
Set Rs=Conn.Execute("Select PostTime From newsinfo Where ID="&ID)
Str="["&Right(Year(Rs("PostTime")),4)&"-"&Right("0"&Month(Rs("PostTime")),2)&"-"&Right("0"&Day(Rs("PostTime")),2)&"]"
Rs.Close
Set Rs=Nothing
GetPostTimeL=Str
...
Function GetPostTimeL2(ID)
Dim Rs,Str
Set Rs=Conn.Execute("Select PostTime From shopinfo Where ID="&ID)
Str="["&Right(Year(Rs("PostTime")),4)&"-"&Right("0"&Month(Rs("PostTime")),2)&"-"&Right("0"&Day(Rs("PostTime")),2)&"]"
Rs.Close
Set Rs=Nothing
GetPostTimeL2=Str
...
'返回值：字符串
'**************************************************
Function GetNewsN(Strsql,TableName,PageSize,url,length,CAndE,TWidth,TitleWidth)
Dim Rs,Str

Response.Write("<table width='"&TWidth&"' border='0' cellspacing='0' align='left' cellpadding='0'>")
Set Rs=Server.CreateObject("Adodb.RecordSet")
Sql = "Select * From "&TableName&" where 1=1 " &Strsql&" Order By  a.PostTime Desc,a.ID Desc"
Rs.Open Sql,Conn,1,1
...
if Not Rs.Eof or not Rs.bof then
	Do While i<Rs.PageSize
	newsid=base64Encode(rs("a.id")&","&rs("classid"))
	if CAndE=0 then							
		Str=Rs("NewsTitle")
		Response.Write("<tr onMouseOver=this.bgColor='#F1F1F1' onMouseOut=this.bgColor=''>")
		Response.Write("<td width='30' height='20' align='center'>")
		Response.Write("<img src=images/news_08.jpg>")
		Response.Write("</td>")
		Response.Write("<td width='"&TitleWidth&"' align=left class='NewsTile'>")
		Response.Write("<a title="&Rs("NewsTitle")&" href="&url&"?url="&newsid&" target=_blank >")
		if len(Str)>length Then
		Response.Write(Left(Str,length)&"...")
		else
		Response.Write(Str)
		...
		if CAndE=1 then
			Str=Rs("EnNewsTitle")
			Response.Write("<tr onMouseOver=this.bgColor='#EDEEF0' onMouseOut=this.bgColor=''>")
			Response.Write("<td width='20' height='23' align='center' class='NewsEast_208_b'>")
			Response.Write("<img src=images/point3.jpg>")
			Response.Write("</td>")
			Response.Write("<td width='"&TitleWidth&"' align=left class='NewsTile'>")
			Response.Write("<a title="&Rs("EnNewsTitle")&" href="&url&"?url="&newsid&" target=_blank >")
			if len(Str)>length Then
			Response.Write(Left(Str,length)&"...")
			else
			Response.Write(Str)
	...
	Response.Write("</a></td>")
	Response.Write("<td width=90 align=center class='NewsDate'>")
	Response.Write(GetPostTimeL(Rs("a.ID")))
	Response.Write("</td></tr>")
...
'返回值：字符串
'**************************************************
Function GetNewsN_M(Strsql,TableName,PageSize,url,length,CAndE,TWidth,TitleWidth,PicUrl)
Dim Rs,Str
...
Dim Rs,Str
Set Rs=Server.CreateObject("Adodb.RecordSet")
Sql = "Select * From "&TableName&" where 1=1 "&Strsql&" Order By ID Desc"
Rs.Open Sql,Conn,1,1
Page=ReplaceBadChar(Trim(Request("Page")))                                                       
Rs.PageSize = PageSize               
Total=Rs.RecordCount               
...
'返回值：字符串
'**************************************************
Function GetNewContent1(CAndE, TWidth,TableName)
if CAndE = 0 then
	...
	End If
	Response.Write("<tr>")
       Response.Write(" <td height='30' align='right' style='padding-right:50px;'>")
	   Response.Write("【<a href='javascript:window.close()' >关闭本页</a>】")
	   Response.Write("</td>") 
      Response.Write("</tr>")  
	Response.Write("</table>")
else
	...
	Response.Write("<tr>")
       Response.Write(" <td height='30' align='right' style='padding-right:50px;'>")
	   Response.Write("【<a href='javascript:window.close()' >close pages</a>】")
	   Response.Write("</td>") 
      Response.Write("</tr>")  
	Response.Write("</table>")
	end if
...
End Function
'**************************************************
'函数名：GetPageContent222
'作  用：获取简介形式的单页内容
'参  数：FieldName------NavContent
'返回值：字符串
'**************************************************
Function GetPageContent222(TableName,ClassID,FieldName)
Set Rs=Server.CreateObject("Adodb.RecordSet")
				Sql="Select * From ["&TableName&"] Where ClassID='"&ClassID&"' Order By NavOrder Asc"
				Rs.Open Sql,Conn,1,1
				if not Rs.eof then
				    ...
				    aa=LoseHtml(rs("NavContent")) 'LoseHtml：过滤html代码
                    if len(aa)>410 then
                    response.Write(left(aa,408)&"......")
                    else
                    response.Write(aa)
		  		     end if	
					end if
...
Function ENGetPageContent222(TableName,ClassID,FieldName)
Set Rs=Server.CreateObject("Adodb.RecordSet")
				Sql="Select * From ["&TableName&"] Where ClassID='"&ClassID&"' Order By NavOrder Asc"
				Rs.Open Sql,Conn,1,1
				if not Rs.eof then
				    ...
				    aa=LoseHtml(rs("EnNavContent"))
                    if len(aa)>300 then
                    response.Write(left(aa,300)&"...")
                    else
                    response.Write(aa)
		  		     end if	
					end if
...
'连接数据
'***************************************
Function Recordset1(TableName,ClassID,top)
Set Rs=Server.CreateObject("Adodb.RecordSet")
				Sql="Select top "&top&" * From ["&TableName&"] Where ClassID="&ClassID&" Order By posttime desc"
				Rs.Open Sql,Conn,1,1
				Total=Rs.RecordCount 
End Function
'*************************************
...
if Request("action")="Message" then
	...
	Set Rs=Nothing
	if CAndE = 0 then
		Response.Write("<script>alert('感谢您的留言！');location.href='#';</script>")
	else
		if CAndE = 1 then
			Response.Write("<script>alert('Thank you for your message！');location.href='#';</script>")
		end if
	end if
	Response.End()
end if
...
Response.Write("<td>")
Set Rs=Server.CreateObject("Adodb.Recordset")
Sql="select * from [GuestBook] where 1=1 "&StrWhere&" order by ID desc"
Rs.open Sql,Conn,1,1
i=0
Page=ReplaceBadChar(Trim(Request("Page")))                                                       
Rs.PageSize = PageSize               
...
Set Rs=Server.CreateObject("Adodb.Recordset")
Sql="select * from [GuestBook] where 1=1 "&StrWhere&" order by PostTime desc"
Rs.open Sql,Conn,1,1
i=0
Page=ReplaceBadChar(Trim(Request("Page")))                                                       
Rs.PageSize = PageSize               
...
Response.Write("<font style='color:#333333;'>园长回复</font>")
Response.Write("</li>")
...
Function menu(sqlstr,url,forms,forms1,css)
	set rs=server.CreateObject("ADODB.Recordset")
	sql=sqlstr
	rs.open sql,conn,1,1
	Total=rs.RecordCount
	...
	ziduan=rs(forms)
	ids=rs(forms1)
	response.Write("<li class='"&css&"'>") 
	response.Write("<a href='"&url&"?classid="&ids&"' title='"&ziduan&"' >")
	response.Write(ziduan) 
	response.Write("</a>")
	response.Write("</li>")
	rs.movenext()
	If rs.Eof Then Exit Do
...
Function  title(sqlstr,url,pagesizes,lengths)
	...
	titles=rs1("NewsTitle")
	
	ids=rs1("id")
	classids=rs1("classid")
	response.Write("<li>")
	response.Write("<span style='float:right;'>")
	response.Write(GetPostTime2(ids,"newsinfo"))
	response.Write("</span>")  
	response.Write("<a href='"&url&"?classid="&classids&"&id="&ids&"' title='"&titles&"' target='_bank' >")
	if len(titles)>lengths then
	response.Write(left(titles,lengths)&"...")
	else
