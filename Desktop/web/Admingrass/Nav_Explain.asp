...
Select Case Action
Case "Up"
	MinOrder=Conn.Execute("Select Min(NavOrder) From SiteExplain")(0)
	...
	Else
		Conn.Execute("Update SiteExplain Set NavOrder="&NavOrder&" Where NavOrder="&NavOrder-1&"")
	...
	Response.End()
Case "Down"
	MaxOrder=Conn.Execute("Select Max(NavOrder) From SiteExplain")(0)
	NavOrder=ReplaceBadChar(Trim(Request("NavOrder")))
...
<tr>
<td width="92%" style="border-bottom:solid 1px #dde4e9;height:30px;width:80%">当前位置：<a href="Nav_Explain.asp">单页内容维护</a><%=GetNavPath("SiteExplain",ID)%></td>
<td width="8%" style="border-bottom:solid 1px #dde4e9;height:30px;width:20%; text-align:right">
<a href="Nav_ExplainAdd.asp?ID=<%=ID%>">添加内容</a>&nbsp;</td>
</tr>
<tr>
...
<td width="90%" valign="top">1.以下为系统所有单页内容的信息列表。<br />
  2.单击[+]可添加一级内容，单击“名称”添加该内容下面的下一级内容。
    <br />
    3.当说明项中无“级别栏目”时，即可对该条数据进行编辑，删除等操作。
    <br>
...
<th width="30%" class="Right">内容标题<a href="Nav_ExplainAdd.asp?ClassID=<%=ClassID%>">[+]</a></th>
<th width="14%" class="Right">所属类别&nbsp;(类别ID)</th>
<th width="24%" class="Right">说明</th>
<th width="8%" class="Right">排序</th>
<th width="8%" class="Right">状态</th>
<th width="9%">管理操作</th>
</tr>
<%
Set Rs=Server.CreateObject("Adodb.RecordSet")
Sql="Select * From SiteExplain Order By NavOrder asc"
...
<td class="Right"><a href="Nav_ExplainEdit.asp?ID=<%=Rs("ID")%>&Page=<%=Page%>&ParentID=<%=ID%>&ClassID=<%=Rs("ClassID")%>">
<%=Rs("NavTitle")%>
</a><a href="?ID=<%=Rs("ID")%>&Page=<%=Page%>&ParentID=<%=ID%>">(<%=GetSubNavCount("SiteExplain",Rs("ID"))%>)</a></td>
<td class="Right">
<%
	Set Rs2=Server.CreateObject("Adodb.RecordSet")
	Sql2="Select User_NavTitle,User_EnNavTtile From User_PageCategory where ID="&Rs("ClassID")
	Rs2.Open Sql2,Conn,1,1
	if not Rs2.eof then
		if Request.Cookies("CNVP_CMS2")("SiteVersion")="Chiness" then
		Response.Write(Rs2("User_NavTitle")&"("&Rs("ClassID")&")")
		end if
		if Request.Cookies("CNVP_CMS2")("SiteVersion")="English" then
		Response.Write(Rs2("User_EnNavTtile")&"("&Rs("ClassID")&")")
		end if
...
<td class="Right"><a href="?Action=Up&ID=<%=Rs("ID")%>&NavOrder=<%=Rs("NavOrder")%>">上移</a>┊<a href="?Action=Down&ID=<%=Rs("ID")%>&NavOrder=<%=Rs("NavOrder")%>">下移</a></td>
<td class="Right"><%
...
End If
%></td>
<td><a href="Nav_ExplainEdit.asp?ID=<%=Rs("ID")%>&Page=<%=Page%>&ParentID=<%=ID%>&ClassID=<%=Rs("ClassID")%>">编辑</a>┊<a href="?Action=Delete&ID=<%=Rs("ID")%>&Page=<%=Page%>&ParentID=<%=ID%>" onclick="if(!confirm('\u786e\u8ba4\u8981\u5c06\u8be5\u5bfc\u822a\u6761\u4fe1\u606f\u5220\u9664\u5417\uff1f')) return false;">删除</a></td>
</tr>
<%
i=i+1
Rs.MoveNext
