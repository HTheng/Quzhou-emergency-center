...
Select Case Action
Case "Up"
		MinOrder=Conn.Execute("Select Min(JobOrder) From JobInfo")(0)
		If MinOrder="" Then MinOrder="1"
		JobOrder=ReplaceBadChar(Trim(Request("JobOrder")))
		If Cstr(JobOrder)=Cstr(MinOrder) Then
			Response.Write("<script>alert('\u8be5\u4fe1\u606f\u5df2\u7ecf\u5904\u4e8e\u6700\u5934\u90e8\u4e86\uff0c\u65e0\u6cd5\u8fdb\u884c\u987a\u5e8f\u7684\u8c03\u6574\u3002');history.back();</script>")
		Else
			Conn.Execute("Update JobInfo Set JobOrder="&JobOrder&" Where JobOrder="&JobOrder-1&"")
			Conn.Execute("Update JobInfo Set JobOrder="&JobOrder-1&" Where ID="&ID&"")
		...
		Response.End()
Case "Down"
	MaxOrder=Conn.Execute("Select Max(JobOrder) From JobInfo")(0)
	JobOrder=ReplaceBadChar(Trim(Request("JobOrder")))
	If Cstr(JobOrder)=Cstr(MaxOrder) Then
	...
	Response.End()
Case "UnIndex"
	Conn.Execute("Update JobInfo Set JobIndex=1 Where ID="&ID&"")
	Response.Redirect(Request.ServerVariables("HTTP_REFERER"))
	Response.End()
Case "InIndex"
	Conn.Execute("Update JobInfo Set JobIndex=0 Where ID="&ID&"")
	Response.Redirect(Request.ServerVariables("HTTP_REFERER"))
	Response.End()
Case "IsLock"
	...
	For i = LBound(AryID) To UBound(AryID)
		If IsNumeric(AryID(i))=True Then
			Conn.Execute("Update JobInfo Set NewsLock=1 Where ID="&AryID(i)&"")
		End If
	Next
	Conn.Close
	Set Conn=Nothing
...
Case "Delete"
	...
	For i = LBound(AryID) To UBound(AryID)
		If IsNumeric(AryID(i))=True Then
			Conn.Execute("Delete From JobInfo Where ID="&AryID(i)&"")
		End If
	Next
	Conn.Close
	Set Conn=Nothing
...
$(document).ready(function(){
	...
	$("#ClassID").change(function(){
		window.location.href='?ClassID='+$("#ClassID").val()+'&NewsTitle=<%=Server.URLEncode(Jobs)%>';
	});
});
</script>
</head>
...
<tr>
<td style="border-bottom:solid 1px #dde4e9;height:30px;width:80%">当前位置：<a href="Job_List.asp">信息内容维护</a>
<%
%> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
<a href="Job_List.asp">页面刷新</a></td>
...
<th width="23%" class="Right">招聘职位<a href="#" onclick="top.CreateNewTab('Job_ContentAdd.asp?ClassID=<%=ClassID%>','Job_ContentAdd','添加职位')">[+]</a></th>
<th width="25%" class="Right">招聘部门</th>
<th width="13%" class="Right">发布日期</th>
<th width="10%" class="Right">类别</th>
<th width="8%" class="Right">排序</th>
<th width="5%" class="Right">状态</th>
<th width="4%" class="Right"><input type="checkbox" name="chkSelectAll" onclick="doCheckAll(this)" /></th>
<th width="8%"><span class="Right">管理操作 </span></th>
</tr>
<%
Set Rs=Server.CreateObject("Adodb.RecordSet")
Sql="Select * From JobInfo Where 1=1 Order By JobOrder asc"
...
<td class="Right"><a href="Job_ContentEdit.asp?ID=<%=Rs("ID")%>&ClassID=<%=Rs("ClassID")%>"><%=Rs("Jobs")%></a></td>
<td class="Right"><%=Rs("RDepart")%></td>
<td class="Right"><%=ConvertDate(Rs("PostTime"))%></td>
<td class="Right"><%=GetSubNavName("JobClass",Rs("ClassID"))%>&nbsp;</td>
<td class="Right"><a href="?Action=Up&ID=<%=Rs("ID")%>&JobOrder=<%=Rs("JobOrder")%>">上移</a>┊<a href="?Action=Down&ID=<%=Rs("ID")%>&JobOrder=<%=Rs("JobOrder")%>">下移</a></td>
<td class="Right">
<%
If Rs("JobIndex")="0" Then
Response.Write("<a href=""?Action=UnIndex&ID="&Rs("ID")&""">已发布</a>")
Else
Response.Write("<a href=""?Action=InIndex&ID="&Rs("ID")&""" style=""color:red"">末发布</a>")
End If
%></td>
<td class="Right"><input type="checkbox" name="ID" value="<%=Rs("ID")%>" /></td>
<td><span class="Right"><a href="Job_ContentEdit.asp?ID=<%=Rs("ID")%>&amp;Page=<%=Page%>&amp;ClassID=<%=ClassID%>">修改</a>┊<a href="?Action=Delete&amp;ID=<%=Rs("ID")%>&amp;Page=<%=Page%>&amp;ClassID=<%=ClassID%>" onclick="if(!confirm('\u786e\u8ba4\u8981\u5c06\u8be5\u4fe1\u606f\u5220\u9664\u5417\uff1f')) return false;">删除</a></span></td>
</tr>
<%
i=i+1
Rs.MoveNext
...
%>
<tr>
<th colspan="3" style="font-weight:normal">操作：<a href="javascript:Delete();" style="font-weight:normal">删除</a>&nbsp;┊&nbsp;<a href="javascript:ChangeParent();" style="font-weight:normal">转移</a>&nbsp;┊&nbsp;<a href="News_ContentListFind.asp?ClassID=<%=ClassID%>" style="font-weight:normal">查找</a></th>
<th colspan="5" style="font-weight:normal">共<%=Rs.PageCount%>页&nbsp;第<%=Page%>页&nbsp;<%=PageSize%>条/页&nbsp;共<%=Total%>条&nbsp;
  <%if Page=1 then%>
  首 页&nbsp;上一页&nbsp;
  <%Else%>
...
function ChangeParent()
{
    ...
    if (confirm('\u786e\u5b9a\u8981\u66f4\u6539\u9009\u4e2d\u4fe1\u606f\u7684\u6240\u5c5e\u7236\u7c7b\u522b\u5417\uff1f')) {
        window.location.href = 'Job_ContentParent.asp?ID='+l;
    }
}
function Delete() {
    var l = GetAllChecked();
