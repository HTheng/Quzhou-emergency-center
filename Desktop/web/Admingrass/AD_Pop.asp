...
<%
Call ISPopedom(UserName,"AD_Pop")
...
Select Case Action
Case "Up"
	MinOrder=Conn.Execute("Select Min(AdsOrder) From SiteAds Where AdsType='Pop'")(0)
	...
	Else
		Conn.Execute("Update SiteAds Set AdsOrder="&AdsOrder&" Where AdsOrder="&AdsOrder-1&" And AdsType='Pop'")
	...
	Response.End()
Case "Down"
	MaxOrder=Conn.Execute("Select Max(AdsOrder) From SiteAds Where AdsType='Pop'")(0)
	AdsOrder=ReplaceBadChar(Trim(Request("AdsOrder")))
...
<tr>
<td style="border-bottom:solid 1px #dde4e9;height:30px;width:80%">当前位置：<a href="AD_Pop.asp">弹出广告</a></td>
<td style="border-bottom:solid 1px #dde4e9;height:30px;width:20%; text-align:right"><a href="AD_PopAdd.asp">添加弹出广告</a>&nbsp;</td>
</tr>
<tr>
...
<th width="25%" class="Right">广告名称<a href="AD_PopAdd.asp">[+]</a></th>
...
Sql="Select * From SiteAds Where AdsType='Pop' Order By AdsOrder Asc"
...
<td class="Right"><a href="Ad_PopEdit.asp?ID=<%=Rs("ID")%>&Page=<%=Page%>"><%=Rs("AdsName")%></a></td>
...
End If
%></td>
<td><a href="Ad_PopEdit.asp?ID=<%=Rs("ID")%>&Page=<%=Page%>">编辑</a>┊<a href="?Action=Delete&ID=<%=Rs("ID")%>&Page=<%=Page%>" onclick="if(!confirm('\u786e\u8ba4\u8981\u5c06\u8be5\u5f39\u51fa\u5e7f\u544a\u5220\u9664\u5417\uff1f')) return false;">删除</a></td>
</tr>
<%
i=i+1
Rs.MoveNext
