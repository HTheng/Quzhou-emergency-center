...
Select Case Action
Case "Up"
	MinOrder=Conn.Execute("Select Min(NavOrder) From SiteNav Where NavParent="&NavParent&"")(0)
	...
	Else
		Conn.Execute("Update SiteNav Set NavOrder="&NavOrder&" Where NavOrder="&NavOrder-1&" And Navparent="&Navparent&"")
		Conn.Execute("Update SiteNav Set NavOrder="&NavOrder-1&" Where ID="&ID&"")
	...
	Response.End()
Case "Down"
	MaxOrder=Conn.Execute("Select Max(NavOrder) From SiteNav Where NavParent="&NavParent&"")(0)
	NavOrder=ReplaceBadChar(Trim(Request("NavOrder")))
	...
	Response.End()
Case "UnPublic"
	Conn.Execute("Update SiteNav Set NavLock=1 Where ID="&ID&"")
	Response.Redirect(Request.ServerVariables("HTTP_REFERER"))
	Response.End()
Case "InPublic"
	Conn.Execute("Update SiteNav Set NavLock=0 Where ID="&ID&"")
	Response.Redirect(Request.ServerVariables("HTTP_REFERER"))
	Response.End()
Case "Delete"
	...
	For i = LBound(AryID) To UBound(AryID)
		If IsNumeric(AryID(i))=True Then
			Conn.Execute("Delete From SiteNav Where ID In ("&AryID(i)&GetAllChild("SiteNav",AryID(i))&")")
		End If
	Next
	Conn.Close
	Set Conn=Nothing
	Response.Write("<script>alert('\u5220\u9664\u64cd\u4f5c\u6210\u529f\uff0c\u786e\u5b9a\u540e\u8fd4\u56de\u5bfc\u822a\u6761\u5217\u8868\u9875\u9762\u3002');window.location.href='?Page="&Page&"&ID="&ParentID&"';</script>")
	Response.End()
End Select
%>
...
<tr>
<td style="border-bottom:solid 1px #dde4e9;height:30px;width:80%">当前位置：<a href="Nav_Content.asp">导航条内容维护</a><%=GetNavPath("SiteNav",ID)%></td>
<td style="border-bottom:solid 1px #dde4e9;height:30px;width:20%; text-align:right"><a href="Nav_ContentAdd.asp?ID=<%=ID%>">添加导航条</a>&nbsp;</td>
</tr>
<tr>
...
<td width="90%" valign="top">以下为系统所有的网站导航条的信息列表。<br>
...
<th width="25%" class="Right">导航条名称<a href="Nav_ContentAdd.asp?ID=<%=ID%>">[+]</a></th>
<th width="40%" class="Right">导航条说明</th>
<th width="10%" class="Right">导航条排序</th>
<th width="10%" class="Right">导航条状态</th>
<th width="10%">管理操作</th>
</tr>
<%
Set Rs=Server.CreateObject("Adodb.RecordSet")
Sql="Select * From SiteNav Where NavParent="&ID&" Order By NavOrder Asc"
...
<td class="Right"><a href="?ID=<%=Rs("ID")%>"><%=Rs("NavTitle")%></a>(<%=GetSubNavCount("SiteNav",Rs("ID"))%>)</td>
...
End If
%></td>
<td><a href="Nav_ContentEdit.asp?ID=<%=Rs("ID")%>&Page=<%=Page%>&ParentID=<%=ID%>">编辑</a>┊<a href="?Action=Delete&ID=<%=Rs("ID")%>&Page=<%=Page%>&ParentID=<%=ID%>" onclick="if(!confirm('\u786e\u8ba4\u8981\u5c06\u8be5\u5bfc\u822a\u6761\u4fe1\u606f\u5220\u9664\u5417\uff1f')) return false;">删除</a></td>
</tr>
<%
i=i+1
Rs.MoveNext
...
function ChangeParent()
{
    ...
    if (confirm('\u786e\u5b9a\u8981\u66f4\u6539\u9009\u4e2d\u7684\u5bfc\u822a\u6761\u7684\u6240\u5c5e\u7236\u7c7b\u522b\u5417\uff1f')) {
        window.location.href = 'Nav_ContentParent.asp?ID='+l;
    }
}
function Delete() {
    var l = GetAllChecked();
