...
Case "Delete"
	...
	For i = LBound(AryID) To UBound(AryID)
		...
		If IsNumeric(AryID(i))=True Then
			...
			Sql = "Select UpLoadAddress From [NewsInfo] where ID="&AryID(i)
...
End If
%> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
<a href="News_List.asp">页面刷新</a></td>
...
<td width="90%" valign="top">1.以下为所有信息的列表，对于需要优先显示的信息你可以使用排序功能；<br />
  2.单击&quot;[+]&quot;添加信息；<br />
    3.点击&quot;查找&quot;根据信息标题进行模糊查找；<br>
...
<th width="36%" class="Right">信息标题<a href="News_ContentAdd.asp" >[+]</a></th>
<th width="28%" class="Right">类别&nbsp;(类别ID)</th>
<th width="8%" class="Right">排序</th>
<th width="8%" class="Right">首页推荐</th>
<th width="8%">管理操作</th>
</tr>
<%
Set Rs=Server.CreateObject("Adodb.RecordSet")
Sql="Select * From NewsInfo Where 1=1 "&StrSql&" And ClassID In ("&ClassID&GetAllChild("NewsClass",ClassID)&") Order By NewsOrder Desc"
Rs.Open Sql,Conn,1,1
Dim Page
Page=Request("Page")                            
PageSize = 10                                    
...
<td class="Right"><a href="News_ContentEdit.asp?ID=<%=Rs("ID")%>&Page=<%=Page%>&ClassID=<%=Rs("ClassID")%>">
<%
...
<td class="Right">
<%if Request.Cookies("CNVP_CMS2")("SiteVersion")="Chiness" or Request.Cookies("CNVP_CMS2")("SiteVersion")="CAndE" then%>
<%Response.Write(GetSubNavName("NewsClass",Rs("ClassID"))&"&nbsp;("&Rs("ClassID")&")")%>
<%end if%>
<%if Request.Cookies("CNVP_CMS2")("SiteVersion")="English" then%>
<%Response.Write(GetSubNavName2("NewsClass",Rs("ClassID")&"&nbsp;("&Rs("ClassID")&")"))%>
<%end if%>&nbsp;</td>
...
%>
<tr>
<th colspan="3" style="font-weight:normal">操作：<a href="javascript:ChangeParent();" style="font-weight:normal">转移</a>&nbsp;┊&nbsp;<a href="News_ContentListFind.asp?ClassID=<%=ClassID%>" style="font-weight:normal">查找</a></th>
