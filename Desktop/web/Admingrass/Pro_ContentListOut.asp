...
<tr>
<td style="border-bottom:solid 1px #dde4e9;height:30px;width:80%">当前位置：<a href="Pro_ContentListOut.asp">下架商品维护</a>
<%
If ShopName<>"" Then
...
<td height="80" colspan="2"><table width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  <td width="75%" valign="top">1.以下为所有下架商品的信息列表，如果想对外发布商品您可以进行上架操作；<br />
    2.单击&quot;[+]&quot;添加新商品；<br />
...
Sql="Select * From ShopInfo Where ShopLock=1 "&StrSql&" And ClassID In ("&session("ClassID")&GetAllChild("ShopClass",session("ClassID"))&") Order By ShopOrder Desc"
else
Sql="Select * From ShopInfo Where ShopLock=1 And ClassID In ("&session("ClassID")&GetAllChild("ShopClass",session("ClassID"))&") Order By ShopOrder Desc"
end if
Rs.Open Sql,Conn,1,1
Dim Page
Page=Request("Page")                    
...
<th width="32%" class="Right">商品名称<a href="#" onclick="top.CreateNewTab('Pro_ContentAdd.asp?ClassID=<%=ClassID%>','Pro_ContentAdd','添加商品')">[+]</a></th>
<th width="35%" class="Right">商品类别</th>
<th width="8%" class="Right">商品排序</th>
<th width="6%" class="Right">首页推荐</th>
<th width="7%">管理操作</th>
</tr>
...
<td class="Right"><a href="Pro_ContentEdit.asp?ID=<%=Rs("ID")%>&Page=<%=Page%>&ClassID=<%=ClassID%>&FileName=Pro_ContentListOut&ShowType=<%=ShowType%>&ShopName=<%=Server.URLEncode(ShopName)%>">
  <%
...
%>
<tr>
<th colspan="3" style="font-weight:normal">操作：<a href="javascript:UnLock();" style="font-weight:normal">上架</a>&nbsp;┊&nbsp;<a href="javascript:ChangeParent();" style="font-weight:normal">转移</a>&nbsp;┊&nbsp;<a href="Pro_ContentListOutFind.asp?ClassID=<%=ClassID%>&ShowType=<%=ShowType%>" style="font-weight:normal">查找</a></th>
